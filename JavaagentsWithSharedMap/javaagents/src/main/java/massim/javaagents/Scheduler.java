...
public class Scheduler implements AgentListener, EnvironmentListener{
    ...
    private AgentGroupCoordinator agentGroupCoordinator = new AgentGroupCoordinator();
    ...
    void setEnvironment(EnvironmentInterface ei) {
        ...
        for (AgentConf agentConf: agentConfigurations) {
            ...
            switch(agentConf.className){
                case "MassimTeam4Agent":
                    agent = new MassimTeam4Agent(agentConf.name, mailService);
                    break;
    ...
    void step() {
        ...
        agents.values().forEach(ag -> {
            try {
                ...
                eis.getPercepts(ag.getName()).values().forEach(pUpdate -> {
                    System.out.println("perceptupdate: " + pUpdate.toString());
                    LOG.info("perceptupdate: " + pUpdate.toString());
        ...
        newPerceptAgents.forEach(massimTeam4agent -> {
            ((MassimTeam4Agent) massimTeam4agent).updateStatesByPerception();
        });

        if (!newPerceptAgents.isEmpty()) {
            agentGroupCoordinator.getAgentGroupCoordinator().evaluateEncounters();
        }

        Set<TaskPlanner> taskPlanners = new HashSet<>();
        newPerceptAgents.forEach(massimTeam4agent -> {
//           if( ((MassimTeam4Agent) massimTeam4agent).isPlanTasks() ) {
                taskPlanners.add(((MassimTeam4Agent) massimTeam4agent).getTaskPlanner());
//           }
        });

        taskPlanners.forEach(planner -> planner.planTaskforAgentsWithRuleSet());

        System.out.println("n new percepts agents: " + newPerceptAgents.size());
    ...
    public void updateAgentStatesByPerception() {
        agents.values().forEach(massimTeam4agent -> {
            ((MassimTeam4Agent) massimTeam4agent).updateStatesByPerception();
        });
    }
